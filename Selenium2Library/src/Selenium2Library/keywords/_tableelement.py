...
class _TableElementKeywords(KeywordGroup):
    ...
    def get_table_cell(self, table_locator, row, column, loglevel='INFO'):
        ...
        if table is not None:
            ...
            if row_index < len(rows):
                ...
                raise AssertionError("The row_index:%s out of row length."%row_index)
        else:
			raise AssertionError("The table with locator %s could not be found."%table_locator)
    ...
    def table_get_row_by_text(self,table_locator, texts, column, loglevel='INFO'): 
        """Find a specific string(texts) in a particular column(column) in the table,
           if find, Returns the row number.If find nothing, Raise Error
        ...
        if table is not None:           
            ...
            while row_index < len(rows):
                columns = rows[row_index].find_elements_by_tag_name('th')
                if column_index >= len(columns): columns.extend(rows[row_index].find_elements_by_tag_name('td'))
                if column_index < len(columns) and (columns[column_index].text).encode('utf-8') == texts.encode('utf-8'):
                    return row_index + 1
                row_index += 1
        self.log_source(loglevel)
        raise AssertionError("Cell in table with text:%s could not be found." % texts ) 
    
    def table_get_row_nums(self,table_locator): 
